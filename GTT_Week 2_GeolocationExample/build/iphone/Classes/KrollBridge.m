...
NSString * GTT_Week_2_GeolocationExample$ModuleRequireFormat = @"(function(exports){"
...
@implementation GTT_Week_2_GeolocationExampleObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_gtt_week_2_geolocationexample);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_gtt_week_2_geolocationexample gc];
}
...
{
	// create GTT_Week_2_GeolocationExample global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "GTT_Week_2_GeolocationExample" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_gtt_week_2_geolocationexample = [[GTT_Week_2_GeolocationExampleObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_gtt_week_2_geolocationexample];
	
	NSString *_gtt_week_2_geolocationexampleNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _gtt_week_2_geolocationexampleNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_gtt_week_2_geolocationexample valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:GTT_Week_2_GeolocationExample$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.gtt_week_2_geolocationexample.kroll" 
