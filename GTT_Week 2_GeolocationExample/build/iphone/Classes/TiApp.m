...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kGTT_Week_2_GeolocationExampleUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kGTT_Week_2_GeolocationExampleUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a GTT_Week_2_GeolocationExample app using the node.js CLI
