...
//Rare exceptions to indicate a bug in the _gtt_week_2_geolocationexample code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_gtt_week_2_geolocationexample = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_gtt_week_2_geolocationexample]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.gtt_week_2_geolocationexample.module"
