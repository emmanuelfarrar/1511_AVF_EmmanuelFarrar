...
NSString * Week1_CommonJS_Example$ModuleRequireFormat = @"(function(exports){"
...
@implementation Week1_CommonJS_ExampleObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_week1_commonjs_example);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Week1_CommonJS_Example global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Week1_CommonJS_Example" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_week1_commonjs_example = [[Week1_CommonJS_ExampleObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_week1_commonjs_example];
	
	NSString *_week1_commonjs_exampleNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _week1_commonjs_exampleNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_week1_commonjs_example valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Week1_CommonJS_Example$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.week1_commonjs_example.kroll" 
