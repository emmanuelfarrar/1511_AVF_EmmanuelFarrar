...
NSString * Week1_RemoteData_Example$ModuleRequireFormat = @"(function(exports){"
...
@implementation Week1_RemoteData_ExampleObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_week1_remotedata_example);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Week1_RemoteData_Example global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Week1_RemoteData_Example" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_week1_remotedata_example = [[Week1_RemoteData_ExampleObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_week1_remotedata_example];
	
	NSString *_week1_remotedata_exampleNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _week1_remotedata_exampleNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_week1_remotedata_example valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Week1_RemoteData_Example$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.week1_remotedata_example.kroll" 
