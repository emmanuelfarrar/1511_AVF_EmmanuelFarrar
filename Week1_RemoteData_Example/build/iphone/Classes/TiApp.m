...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kWeek1_RemoteData_ExampleUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kWeek1_RemoteData_ExampleUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Week1_RemoteData_Example app using the node.js CLI
