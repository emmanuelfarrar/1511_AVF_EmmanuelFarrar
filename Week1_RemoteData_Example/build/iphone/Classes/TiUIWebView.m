...
static NSString * const kWeek1_RemoteData_ExampleJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
- (NSString *)_week1_remotedata_exampleInjection
{
	if (pageToken==nil) {
	...
	[html appendString:kWeek1_RemoteData_ExampleJavascript];
...
+ (NSString *)content:(NSString *)content withInjection:(NSString *)injection
{
	...
	// attempt to make well-formed HTML and inject in our Week1_RemoteData_Example bridge code
	   ...
	   baseURL:(NSURL*)baseURL
{
	...
	content = [[self class] content:content withInjection:[self _week1_remotedata_exampleInjection]];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
	...
	if ([scheme hasPrefix:@"http"] || [scheme isEqualToString:@"ftp"]
		...
		if ([scheme isEqualToString:@"file"] || [scheme isEqualToString:@"app"]) {
			[NSURLProtocol setProperty:[self _week1_remotedata_exampleInjection] forKey:kContentInjection inRequest:(NSMutableURLRequest *)request];
		}
		return YES;
	}
