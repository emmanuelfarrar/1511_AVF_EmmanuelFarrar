...
NSString * Week_1_Project___Geolocation$ModuleRequireFormat = @"(function(exports){"
...
@implementation Week_1_Project___GeolocationObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_week_1_project___geolocation);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_week_1_project___geolocation gc];
}
...
{
	// create Week_1_Project___Geolocation global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Week_1_Project___Geolocation" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_week_1_project___geolocation = [[Week_1_Project___GeolocationObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_week_1_project___geolocation];
	
	NSString *_week_1_project___geolocationNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _week_1_project___geolocationNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_week_1_project___geolocation valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Week_1_Project___Geolocation$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.week_1_project___geolocation.kroll" 
