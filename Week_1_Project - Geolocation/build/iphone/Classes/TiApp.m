...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kWeek_1_Project___GeolocationUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kWeek_1_Project___GeolocationUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Week_1_Project___Geolocation app using the node.js CLI
