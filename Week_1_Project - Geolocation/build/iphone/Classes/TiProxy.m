...
//Rare exceptions to indicate a bug in the _week_1_project___geolocation code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_week_1_project___geolocation = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_week_1_project___geolocation]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.week_1_project___geolocation.module"
